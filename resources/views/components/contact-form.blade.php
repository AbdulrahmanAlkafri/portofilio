...
<div class="bg-white p-6">
    <h2 class="text-4xl font-bold text-gray-600 mb-4 text-center">كن على تواصل معنا</h2>
    <form action="{{ route('contacts.store') }}" method="POST" class="space-y-8">
        ...
        <div class="mb-4">
            <input type="text" name="phone_number" id="phone_number" placeholder="رقم الهاتف"
                class="mt-1 block w-1/3 bg-gray-100 placeholder:text-black placeholder:text-sm mx-auto border border-gray-300 rounded-md p-2 focus:outline-none focus:ring focus:ring-orange-300"
                required>
            @error('phone_number')
            <span class="text-red-500 text-sm">{{ $message }}</span>
        ...
        <div class="mb-4">
            <textarea name="subject" id="subject" placeholder="رسالتك لنا "
                class="mt-1 block w-1/3 bg-gray-100 placeholder:text-black placeholder:text-sm mx-auto border border-gray-300 rounded-md p-2 focus:outline-none focus:ring focus:ring-orange-300"
                required></textarea>
            @error('subject')
            <span class="text-red-500 text-sm">{{ $message }}</span>
        ...
        <button type="submit"
            class="block w-1/3 mx-auto font-bold bg-orange-500 py-2 text-orange-50 rounded-md hover:bg-orange-600">ارسال
            الرساله</button>
    </form>
</div>